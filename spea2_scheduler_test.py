...
class BasicScheduler:
    ...
    def fitness_assignment(self, population, archive, node_cpu_usage):
        if population.size == 0 or archive.size == 0:
        ...
        # Handle edge case where combined has fewer elements than self.k
        if N <= self.k:
            distances = np.zeros(N)
        else:
            distances = np.array([
                1 / (np.sort(np.linalg.norm(combined - combined[i], axis=1))[self.k] + 1e-6)
                for i in range(N)
            ])
        ...
        if cpu_values.size > 0 and np.max(cpu_values) > np.min(cpu_values):
            cpu_values = (cpu_values - np.min(cpu_values)) / (np.max(cpu_values) - np.min(cpu_values) + 1e-6)
        else:
            cpu_values = np.zeros_like(cpu_values)
