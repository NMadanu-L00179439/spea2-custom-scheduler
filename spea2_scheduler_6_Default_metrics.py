...
class BasicScheduler:
    ...
    def fetch_node_metrics(self, api):
        ...
        # Replace this with actual code to get node metrics if you have an alternative source
        node_cpu_usage = {f'node{i}': np.random.rand() for i in range(10)}
        mem_usage = {f'node{i}': np.random.randint(5000, 16000) for i in range(10)}  # In MiB
        print(f"Node CPU Usage (simulated): {node_cpu_usage}", flush=True)
        print(f"Node Memory Usage (simulated): {mem_usage}", flush=True)
